...
function stage1Materials(createMaterialBuilder, registerMaterialProcess) {
    ...
    let virtual_gold = createMaterialBuilder('virtual_gold')
        .setCraftable(false).setSortOrder(sortOrder++).setTier(1)
        .addStats()
        .addHead(db, 24.0, 9.0, 'minecraft:netherite')
        .addHandle(0, 0.3, 0.05, 0.1)
        .addPlatingHelmet(5.0, db * 0.7, 0.05, 1.0)
        .addPlatingChestplate(7.0, db * 1.0, 0.05, 1.0)
        .addPlatingLeggings(6.0, db * 0.9, 0.05, 1.0)
        .addPlatingBoots(3.0, db * 0.6, 0.05, 1.0)
        .addPlatingShield(db * 0.9, 0.05, 1.0)
        .addBinding()
        .addMaille()
        .addLimb(0.1, -0.25, db, 1.8)
        .addGrip(0, 0, 9)
        .addTraits()
    ...
    let alpha_framework = createMaterialBuilder('alpha_framework')
        .setCraftable(true).setSortOrder(sortOrder++).setTier(1)
        .addStats()
        .addHead(db, 42.0, 10.0, 'minecraft:netherite')
        .addHandle(0.1, 0.35, 0.05, 0.15)
        .addPlatingHelmet(5.0, db * 0.7, 0.05, 3.0)
        .addPlatingChestplate(7.0, db * 1.0, 0.05, 3.0)
        .addPlatingLeggings(6.0, db * 0.9, 0.05, 3.0)
        .addPlatingBoots(4.0, db * 0.6, 0.05, 3.0)
        .addBinding()
        .addMaille()
        .addShieldCore()
        .addTraits()
        .addLimb(0.2, -0.2, db, 3.0)
        .addGrip(0.2, 0.1, 14)
        .addDefaultTrait(2, 'tconstruct:enhanced')
        .addDefaultTrait(1, 'tconstruct:ductile')
