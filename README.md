# 🎮 我的世界·帕鲁梦整合包 🌌  
# 🎮 Minecraft Paluloom Modpack 🌌  

[![GitHub Issues](https://img.shields.io/github/issues/Tonywww2/Palmon-Modpack)](https://github.com/Tonywww2/Palmon-Modpack/issues)  

---

## 🌟 简介 | Introduction  
深度魔改整合包，基于 `1.20.1`  
A heavily modified Minecraft modpack, built on `1.20.1`

---

## ✨ 功能特色 | Features  
### 🦄 核心玩法 | Gameplay  
- 匠魂与多阶段任务系统  
- TConstruct & multi-stage quest system  

### 🛠️ 技术支持 | Technical   
- 兼容主流光影与多人联机  （服务端在做了）
- Compatible with shaders & multiplayer  

---


## 🐛 问题反馈 | Issue Reporting 
- BUG/优化/缺少食物兼容等问题请创建Issue，并加上对应的label
- Bugs/Optimization Issues/Missing food compatibility Issues, please create an Issue and mark it with label

---

## 📥 下载 | Downloads  
- [CurseForge](www.curseforge.com/) 
